...
impl GitHub {
    pub fn new(repos: Vec<RepoConfig>, token: Option<String>, http: HttpClient) -> Self {
        Self { repos, token, http }
    }
}
    ...
    fn check(&self, storage: &mut Storage) -> Option<Notification> {
        ...
        for repo in &self.repos {
            if repo.watch.commits
                && let Some(entry) = check_commits(repo, storage, self.token.as_deref(), &self.http)
            {
                entries.push(entry);
            }
            if repo.watch.prs
                && let Some(entry) = check_prs(repo, storage, self.token.as_deref(), &self.http)
            {
                entries.push(entry);
            }
            if repo.watch.releases
                && let Some(entry) =
                    check_releases(repo, storage, self.token.as_deref(), &self.http)
            {
                entries.push(entry);
            }
        }
...
fn resolve_branches(
    ...
    match &repo.branches {
        BranchesConfig::Default => {
            ...
            let branch =
                fetch_default_branch(repo, token, http).unwrap_or_else(|| "main".to_string());
            storage.set(&cache_key, &branch);
            vec![branch]
        }
        BranchesConfig::All => fetch_all_branches(repo, token, http).unwrap_or_default(),
...
fn fetch_default_branch(
    ...
    let json = api_get(&path, token, http).ok()?;
    json.get("default_branch")?.as_str().map(|s| s.to_string())
}

fn fetch_all_branches(
    repo: &RepoConfig,
    token: Option<&str>,
    http: &HttpClient,
) -> Option<Vec<String>> {
...
fn check_commits(
    ...
    let branches = resolve_branches(repo, storage, token, http);
    ...
    for branch in &branches {
        ...
        let latest = fetch_latest_commit_sha(repo, branch, token, http)?;
        ...
        let commits = match stored_sha {
            Some(ref old) if old != &latest => {
                fetch_commits_between(repo, old, &latest, branch, token, http).unwrap_or_default()
            }
        ...
        let filtered: Vec<&CommitDetail> = commits
            ...
            .filter(|c| {
                ...
                !repo
                    .ignore_authors
                    .iter()
                    .any(|ignored| Some(ignored.as_str()) == login)
            })
            .collect();
...
fn fetch_commits_between(
    ...
    if json.get("commits").is_none() {
        return fetch_recent_commits(repo, branch, token, 30, http);
    }
...
fn check_prs(
    ...
    let prs = fetch_open_prs(repo, token, http)?;
...
fn check_releases(
    ...
    let releases = fetch_releases(repo, token, http)?;
